...
float4 psmain(PS_INPUT input) : SV_TARGET
{
	...
	//Specular Light
	float ks = 0.0;							//Constant specular light amount
	...
	float specularity = 30.0;				//Decreasing this makes model more shiny and increasing makes it less shiny, because maths
	float amount_specular_light = pow(max(0.0, dot(reflected_light, direction_to_camera)), specularity);
